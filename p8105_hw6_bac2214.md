...
Here are some plots for Bootstrap Distribution of R-squared &
Distribution of Bootstrap Estimates of log(beta1 \* beta2):
...
- The CI for r_squared is 0.89 to 0.94,
- The Bootstrap Distribution of log(beta1 \* beta2) is skewed right
  significantly.
- The CI for log(beta1 \* beta2) is -9.06 to -4.62.

## Question 3

I started this question off by importing the dataset and making factors
...
- gaweeks: The duration of the pregnancy is a strong predictor of birth
  weight; full-term babies typically weigh more than pre-term babies.

- smoken: Smoking has been linked to lower birth weight, so including
  this variable can help account for its negative effect on the baby’s
  weight.

- momage: Maternal age can be a factor in birth weight, with teen
  mothers and older mothers often having different risks for low birth
  weight.

- parity: The number of previous pregnancies can affect birth weight, as
  the uterus environment changes with each pregnancy.

- wtgain: Adequate weight gain during pregnancy is critical for fetal
  growth. Insufficient or excessive weight gain can both negatively
  affect birth weight.

- My interaction term accounts for the fact that the effect of
  gestational age on birth weight might differ for babies of smokers
  compared to non-smokers.

Below is my plot of residuals against fitted values \# Model
diagnostics:
...
![](p8105_hw6_bac2214_files/figure-gfm/unnamed-chunk-9-1.png)<!-- -->

Here I compare my proposed model to two other models in terms of
...
```
    ...
    ## # A tibble: 1 × 3
    ##   error_proposed error_model1 error_model2
    ##            <dbl>        <dbl>        <dbl>
    ## 1            NaN          NaN          NaN
